...
// ===== 파서 상태머신 =====
enum State { WAIT_HEAD, READ_LEN, READ_BODY, READ_CRC, READ_TAIL };
...
void processFrame() {
  ...
  switch (cmd) {
    case CMD_MOTOR: {
      ...
      if (dir == 0) motorsForward(speed);
      else          motorsReverse(speed);
      Serial.print("[OK] MOTOR dir="); Serial.print(dir);
...
void parseByte(uint8_t b) {
  switch (state) {
    ...
    case READ_BODY:
      buf[bodyIdx++] = b;
      if (bodyIdx >= bodyLen) state = READ_CRC;
      break;
